      ...
      <div className="flex flex-wrap justify-center gap-4">
        {inputArray.map((element, index) => {
          ...
          return (
            <div
              key={index}
              className={`rounded-lg font-[75px] h-32 w-24 m-[1%] text-center text-white focus:outline-none pt-11 px-3 overflow-hidden bg-[#1e2d2f] ${sorted ? 'border border-solid border-green-500 element-transition' : 'border border-solid border-red-500 element-transition'} transition-all duration-300 m-4`}
            >
              {element}
            </div>
          );
